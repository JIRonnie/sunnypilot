...
from selfdrive.controls.lib.drive_helpers import V_CRUISE_INITIAL, update_v_cruise, initialize_v_cruise, update_v_cruise_honda
...
IGNORE_PROCESSES = {"uploader", "deleter", "loggerd", "logmessaged", "tombstoned", "statsd",
                    "logcatd", "proclogd", "clocksd", "updated", "timezoned", "manage_athenad", "laikad", "mapd"} | \
...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    self.toyota_force_sng = params.get_bool("ToyotaForceSnG")
    ...
    self.custom_torque = params.get_bool("CustomTorqueLateral")
    ...
    if not self.CP.customStockLongAvailable:
  ...
  def update_events(self, CS):
    ...
    if (CS.gasPressed and not self.CS_prev.gasPressed and self.disengage_on_accelerator) or \
      (CS.brakePressed and (not self.CS_prev.brakePressed or not CS.standstill) and not self.mads_ndlob):
  ...
  def state_transition(self, CS):
    ...
    if CS.cruiseState.available:
      ...
      if not self.CP.pcmCruise or (self.CP.pcmCruise and not self.CP.pcmCruiseSpeed):
        if CS.cruiseState.enabled and self.cruiseState_enabled_last:
          if self.CP.carName == "honda":
            for b in CS.buttonEvents:
              if b.pressed:
                ...
                if b.type == car.CarState.ButtonEvent.Type.accelCruise:
                  ...
                  self.decel_pressed = False
            self.v_cruise_kph = update_v_cruise_honda(self.v_cruise_kph if self.is_metric else int(round((float(self.v_cruise_kph) * 0.6233 + 0.0995))),
                                                      CS.buttonEvents, self.enabled, cur_time, self.accel_pressed,
                                                      self.decel_pressed, self.accel_pressed_last, self.decel_pressed_last, self.fastMode, self.is_metric, self.CP.pcmCruiseSpeed)
            ...
            self.v_cruise_kph = update_v_cruise(self.v_cruise_kph, CS.vEgo, CS.gasPressed, CS.buttonEvents,
                                                self.button_timers, self.enabled, self.is_metric, self.CP.pcmCruiseSpeed)
            self.v_cruise_cluster_kph = self.v_cruise_kph
      else:
    ...
    if self.state != State.disabled:
      ...
      if self.events.any(ET.USER_DISABLE):
        ...
        if self.state == State.enabled:
          if (not self.CP.pcmCruise or (self.CP.pcmCruise and not self.CP.pcmCruiseSpeed)) and CS.cruiseState.enabled and not self.cruiseState_enabled_last:
  ...
  def state_control(self, CS):
    ...
    CC.longActive = self.active and CS.cruiseState.enabled and not CS.brakePressed and not self.events.any(ET.OVERRIDE_LONGITUDINAL) and not self.events.any(ET.NO_ENTRY)
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    CC.cruiseControl.cancel = CS.cruiseState.enabled and (not self.enabled_long or CS.brakePressed)
