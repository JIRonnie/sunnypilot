...
def update_v_cruise(v_cruise_kph, v_ego, gas_pressed, buttonEvents, button_timers, enabled, metric, pcm_cruise_speed):
  ...
  if button_type:
    ...
    if not pcm_cruise_speed:
      v_cruise_min = 30 if metric else 32
    else:
...
def update_v_cruise_honda(v_cruise_kph, buttonEvents, enabled, cur_time, accel_pressed, decel_pressed,
                          accel_pressed_last, decel_pressed_last, fastMode, metric, pcm_cruise_speed):
  ...
  if enabled:
    ...
    if not pcm_cruise_speed:
      ...
      v_cruise_min = V_CRUISE_MIN_HONDA
