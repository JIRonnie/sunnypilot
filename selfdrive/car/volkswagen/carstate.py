...
class CarState(CarStateBase):
  ...
  def update(self, pt_cp, cam_cp, ext_cp, trans_type):
    ...
    ret.brakeLights = bool(pt_cp.vl["ESP_05"]['ESP_Status_Bremsdruck'] or ret.brakePressed or ret.parkingBrake)
    ...
    if ret.cruiseState.available:
      if not self.CP.pcmCruise or not self.CP.pcmCruiseSpeed:
        for b in self.button_events:
          ...
          elif b.type in (ButtonType.resumeCruise, ButtonType.accelCruise) and not b.pressed:
            self.accEnabled = True
      if self.enable_mads:
    ...
    if not self.CP.pcmCruiseSpeed:
      ret.cruiseState.enabled = ret.cruiseState.enabled and self.accEnabled
