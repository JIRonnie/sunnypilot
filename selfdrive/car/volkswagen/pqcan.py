...
def create_acc_buttons_control(packer, bus, gra_stock_values, counter, cancel=False, resume=False, button_send=0):
  ...
  if button_send != 0 and not (cancel or resume):
    values.update({
      "GRA_Neu_Setzen": 1 if button_send == 4 else 0,
      "GRA_Up_kurz": 1 if button_send == 1 else 0,
      "GRA_Down_kurz": 1 if button_send == 2 else 0,
      "GRA_Recall": 1 if button_send == 3 else 0
    })

  #if buttonStatesToSend is not None and not (cancel or resume):
  #  values.update({
  #    "GRA_Abbrechen": buttonStatesToSend["cancel"],
  #    "GRA_Neu_Setzen": buttonStatesToSend["setCruise"],
  #    "GRA_Up_kurz": buttonStatesToSend["accelCruise"],
  #    "GRA_Down_kurz": buttonStatesToSend["decelCruise"],
  #    "GRA_Recall": buttonStatesToSend["resumeCruise"]
  #  })
