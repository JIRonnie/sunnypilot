...
def create_acc_buttons_control(packer, bus, gra_stock_values, counter, cancel=False, resume=False, button_send=0):
  ...
  if button_send != 0 and not (cancel or resume):
    values.update({
      "GRA_Tip_Setzen": 1 if button_send == 4 else 0,
      "GRA_Tip_Hoch": 1 if button_send == 1 else 0,
      "GRA_Tip_Runter": 1 if button_send == 2 else 0,
      "GRA_Tip_Wiederaufnahme": 1 if button_send == 3 else 0
    })
