...
class CarController:
  ...
  def update(self, CC, CS, ext_bus):
    ...
    if gra_send_ready and not CC.cruiseControl.resume and CS.out.cruiseState.enabled and not CS.out.gasPressed and not self.CP.pcmCruiseSpeed:
      counter = (CS.gra_stock_values["COUNTER"] + 1) % 16
      button_send = 0
      self.cruise_button = self.get_cruise_buttons(CS, self.v_cruise_kph_prev)
      if self.cruise_button is not None:
        if self.acc_type == 0:
          button_send = 1 if self.cruise_button == 1 else 2 if self.cruise_button == 2 else 0
        elif self.acc_type == 1:
          button_send = 3 if self.cruise_button == 1 else 4 if self.cruise_button == 2 else 0
        can_sends.append(self.CCS.create_acc_buttons_control(self.packer_pt, ext_bus, CS.gra_stock_values, counter,
                                                             cancel=CC.cruiseControl.cancel, resume=CC.cruiseControl.resume, button_send=button_send))
