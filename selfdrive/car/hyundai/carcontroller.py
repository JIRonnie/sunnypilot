...
class CarController:
  def __init__(self, dbc_name, CP, VM):
    ...
    self.disengage_lateral = False
    self.madsEnabledPrev = False
    self.disengage_lat_count = 0
  ...
  def update(self, CC, CS):
    ...
    self.madsEnabledPrev = CS.madsEnabled
    ...
    pause_lateral = CS.madsEnabled and not CC.latActive
    ...
    if self.madsEnabledPrev and not CS.madsEnabled:
      self.disengage_lat_count = 0
      self.disengage_lateral = True
    if self.disengage_lateral:
      if self.disengage_lat_count >= 1:
        self.disengage_lat_count = 0
        self.disengage_lateral = False
      else:
        self.disengage_lat_count += cur_time
    else:
      self.disengage_lat_count = 0

    if not (pause_lateral or below_lane_change_speed or self.disengage_lateral):
      self.disengage_blink = cur_time

    disengage_blinking_icon = (pause_lateral or below_lane_change_speed or self.disengage_lateral) and not\
    ...
    if self.CP.carFingerprint in CANFD_CAR:
      ...
      if self.CP.openpilotLongitudinalControl:
        ...
        if self.frame % 2 == 0:
          can_sends.append(hyundaicanfd.create_acc_control(self.packer, self.CP, CC.enabled and CS.out.cruiseState.enabled, accel, stopping, CC.cruiseControl.override,
                                                           set_speed_in_units))
      else:
        # button presses
      ...
      can_sends.append(hyundaican.create_lkas11(self.packer, self.frame, self.car_fingerprint, apply_steer, lat_active,
                                                ...
                                                pause_lateral, below_lane_change_speed, disengage_blinking_icon))
      ...
      if self.frame % 5 == 0 and self.car_fingerprint in (CAR.SONATA, CAR.PALISADE, CAR.IONIQ, CAR.KIA_NIRO_EV, CAR.KIA_NIRO_HEV_2021,
        ...
        can_sends.append(hyundaican.create_lfahda_mfc(self.packer, CC.latActive, pause_lateral, below_lane_change_speed, disengage_blinking_icon, slc_active, round(self.speed_limit * CV.MS_TO_KPH), speed_limit_changed, switching_to_hda))
