...
from selfdrive.car.hyundai.values import HyundaiFlags, CAR, DBC, CANFD_CAR, CAMERA_SCC_CAR, EV_CAR, HYBRID_CAR, LEGACY_SAFETY_MODE_CAR, NON_SCC_CAR, Buttons, CarControllerParams
...
class CarInterface(CarInterfaceBase):
  ...
  def _update(self, c):
    ...
    for b in ret.buttonEvents:
      ...
      if (b.type in (ButtonType.accelCruise, ButtonType.decelCruise, ButtonType.setCruise) and not b.pressed and not self.CP.pcmCruise) or \
        ret.cruiseState.enabled and not self.CS.out.cruiseState.enabled and allow_enable and self.CP.pcmCruise:
         enable_pressed = True
      ...
      if (b.type == ButtonType.cancel and b.pressed and not self.CP.pcmCruise) or \
        (not ret.cruiseState.enabled and self.CP.pcmCruise):
